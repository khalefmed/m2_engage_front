  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10 px-1 sm:px-0">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Campagnes</h1>
      ...
      <div className="relative group">
        ...
        <Input 
          className="pl-10 h-11 sm:h-10 border-slate-200 dark:border-white/10" 
          value={search} 
      ...
      <Card className="border-none shadow-elegant bg-white dark:bg-white/5 overflow-hidden">
        <CardHeader className="border-b dark:border-white/5 bg-slate-50/50 dark:bg-transparent px-4">
        ...
        <CardContent className="p-0">
          <div className="divide-y dark:divide-white/5">
            ...
            {filteredCampaigns.map((campaign) => (
              <div key={campaign.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 hover:bg-slate-50 dark:hover:bg-white/5 transition-all">
                <div className="flex items-start gap-3">
                  <div className={`mt-1 p-2.5 rounded-xl shrink-0 ${ (campaign as any).type === 'sms' ? 'bg-amber-100 text-amber-600 dark:bg-amber-500/10' : 'bg-violet-100 text-violet-600 dark:bg-violet-500/10' }`}>
                    {(campaign as any).type === 'sms' ? <ChatBubbleIcon className="h-5 w-5" /> : <EnvelopeClosedIcon className="h-5 w-5" />}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center flex-wrap gap-2 mb-1">
                      <span className="font-bold text-sm text-slate-900 dark:text-white truncate max-w-[200px]">{campaign.name}</span>
                ...
                <div className="flex items-center gap-2 pt-2 sm:pt-0 border-t sm:border-none dark:border-white/5">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-slate-900/60 backdrop-blur-sm p-0 sm:p-4 animate-in fade-in duration-200">
          <Card className="w-full max-w-3xl bg-white dark:bg-[#0b0b13] shadow-2xl border-none rounded-t-[2rem] sm:rounded-3xl overflow-hidden max-h-[95vh] flex flex-col animate-in slide-in-from-bottom duration-300">
            <CardHeader className="border-b dark:border-white/10 p-5 shrink-0 relative">
              <CardTitle className="text-lg">{editingCampaign ? 'Édition' : 'Nouvelle Campagne'}</CardTitle>
              <button onClick={closeModal} className="absolute right-4 top-4 p-2 text-slate-400 hover:bg-slate-100 dark:hover:bg-white/10 rounded-full"><Cross2Icon className="h-5 w-5"/></button>
            </CardHeader>
            ...
            <CardContent className="p-5 overflow-y-auto flex-1">
              <form onSubmit={handleSubmit} className="space-y-6 pb-6">
                ...
                <div className="flex p-1 bg-slate-100 dark:bg-white/5 rounded-2xl w-full sm:w-fit">
                  <button
                    type="button"
                    onClick={() => setForm({...form, type: 'email'})}
                    className={`flex-1 sm:flex-none px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${form.type === 'email' ? 'bg-white shadow-sm text-violet-600 dark:bg-white/10 dark:text-white' : 'text-slate-500'}`}
                  >
                    Email
                  </button>
                  <button
                    ...
                    className={`flex-1 sm:flex-none px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${form.type === 'sms' ? 'bg-white shadow-sm text-amber-600 dark:bg-white/10 dark:text-white' : 'text-slate-500'}`}
                  >
                    SMS
                  </button>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea 
                    className="w-full min-h-[120px] sm:min-h-[150px] p-4 border rounded-2xl text-sm dark:bg-white/5 dark:border-white/10 focus:ring-2 focus:ring-violet-500 outline-none resize-none"
                    value={form.content} 
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1">
                    {segmentsQuery.data?.map(s => (
                      <label key={s.id} className={`flex items-center gap-3 p-4 border rounded-2xl cursor-pointer transition-all ${form.segments.includes(s.id) ? 'bg-violet-50 border-violet-200 dark:bg-violet-500/10 dark:border-violet-500/30' : 'hover:bg-slate-50 dark:border-white/5'}`}>
                        <input 
                          type="checkbox" 
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t dark:border-white/10">
