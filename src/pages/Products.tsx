      ...
      <Card className="border-none shadow-elegant overflow-hidden bg-white dark:bg-white/5">
        <CardHeader className="px-4 py-5 border-b dark:border-white/5 bg-slate-50/30 dark:bg-transparent">
          <div className="flex flex-col gap-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 rounded-2xl bg-violet-50/50 dark:bg-white/5 border border-violet-100 dark:border-white/10 shadow-sm">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-violet-600 dark:text-violet-400 flex items-center gap-2">
                  <BarChartIcon className="w-3 h-3" /> Trier par Performance
                </label>
                <select 
                  value={performanceOrder}
                  onChange={(e) => setPerformanceOrder(e.target.value)}
                  className="w-full h-10 px-3 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 text-xs font-bold shadow-sm focus:ring-2 focus:ring-violet-500 transition-all outline-none"
                >
            ...
            {products.map((product) => (
              ...
              >
                <div className="min-w-0 flex-1 pr-4">
                  ...
                  <div className="flex items-center flex-wrap gap-x-3 gap-y-1">
                    ...
                    <div className="flex items-center gap-1">
                       <span className="text-[11px] font-bold text-violet-600 dark:text-violet-400">{product.total_sales}</span>
                       <span className="text-[10px] text-slate-400 uppercase font-bold tracking-tighter">unités</span>
                    </div>
                    <div className="flex items-center gap-1">
                       <span className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400">{formatCurrency(product.total_revenue)}</span>
                ...
                <div className="shrink-0 flex items-center gap-4">
                  <p className="hidden sm:block text-xs font-black text-slate-700 dark:text-slate-300">
            ...
            <div className="p-4 bg-slate-50/50 dark:bg-white/5 border-t dark:border-white/5">
              ...
              <div className="flex flex-wrap gap-2">
                {categories.map((category) => (
                  <Badge key={category} variant="default" className="bg-white dark:bg-white/10 text-slate-600 dark:text-slate-300 border-none shadow-sm px-3 py-1 text-[10px] font-bold">
                    {category}
                  </Badge>
                ))}
              </div>
  ...
  return (
    <button
      ...
      className={`px-5 py-2 rounded-full text-[11px] font-black uppercase tracking-tighter transition-all whitespace-nowrap ${
        ...
        : 'bg-slate-100 text-slate-500 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10'
      }`}
    >
      {children}
    </button>
...
function DetailBox({ label, value, isHighlight }: { label: string, value?: string, isHighlight?: boolean }) {
  return (
    <div className="p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 shadow-sm">
